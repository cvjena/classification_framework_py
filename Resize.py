...
class Resize(IAlgorithm):
    def __init__(self, target_shape, mode='stretch'):
        ...
        assert mode in ['stretch','resize_smaller_side', 'resize_larger_side'], 'Mode {} does not exist in Resize()'.format(mode)
        
        if mode == 'stretch':
            assert self.target_shape.size >= 2, 'Mode "stretch" requires the target shape to be at least 2 dimensional.'
        elif mode in ['resize_larger_side','resize_smaller_side']:
